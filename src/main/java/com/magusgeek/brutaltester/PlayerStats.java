...
public class PlayerStats {
	...
	public static PlayerStats merge(PlayerStats t1, PlayerStats t2) {
		assert (t1.n == t2.n);
		var p = new PlayerStats(2 * t1.n);

		for (int i = 0; i < t1.n; ++i) {
			for (int j = 0; j < t2.n; ++j) {
				for (int k = 0; k < 3; k++) {
					p.stats[i][j][k] = t1.stats[i][j][k] + t2.stats[i][j][k];
				}
				p.global[i][j] = t1.global[i][j] + t2.global[i][j];
			}
		}
